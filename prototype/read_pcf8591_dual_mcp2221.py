#!/usr/bin/env python3
"""
Read 8 analog channels using two Adafruit PCF8591 boards via an MCP2221 USB-to-I2C bridge.

This uses CircuitPython Blinka to talk to MCP2221 (HID). It does not require
Linux i2c-dev; works on macOS/Windows/Linux.

Setup (host):
  pip install adafruit-blinka hidapi
  # Linux udev (optional, for non-root access):
  #   echo 'SUBSYSTEM=="hidraw", ATTRS{idVendor}=="04d8", ATTRS{idProduct}=="00dd", MODE="0666"' | \
  #     sudo tee /etc/udev/rules.d/99-mcp2221.rules && sudo udevadm control --reload-rules && sudo udevadm trigger

Examples:
  python prototype/read_pcf8591_dual_mcp2221.py
  python prototype/read_pcf8591_dual_mcp2221.py --loop --hz 2 --vcc 5.0 --addr-a 0x4A --addr-b 0x4B
"""
...
def main(argv: Optional[list[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Read two PCF8591 boards over I2C via MCP2221 (Blinka)")
    ...
    # Force Blinka to use MCP2221 backend
    os.environ.setdefault("BLINKA_MCP2221", "1")
    try:
        import board  # type: ignore
        import busio  # type: ignore
    except Exception as e:
        print("Blinka not available. Install with: pip install adafruit-blinka hidapi", file=sys.stderr)
        return 2

    try:
        i2c = busio.I2C(board.SCL, board.SDA)
    except Exception as e:
        print(f"Failed to open MCP2221 I2C: {e}", file=sys.stderr)
        return 2
