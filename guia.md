# Fruitful Search Guide

This document captures the product philosophy, behaviors, and developer notes for Fruitful Search.

## Purpose & Users
- Users: makers and hobbyists seeking components to complete a project.
...
- Artifacts (planned): raw `data/raw/adafruit_catalog.json`, processed `data/processed/catalog.parquet`, indexes `indexes/lexical/` (SQLite FTS5) and `indexes/semantic/` (FAISS).
...
- Compatibility: accessories for Feather/QT Py/Metro via basic rule-based mapping.
...
## Filters & Constraints
- Stock: in-stock default with toggle to include OOS; optional legacy/discontinued visibility.
...
- Commands: `:open <pid>` opens product URL. `:help` for commands. `:q` quits.
- Notes: offline-only; reads `indexes/lexical/index.sqlite` (FTS5). No changes to the TUI.

## Troubleshooting
- Headless/WSL browser issues: if `:open` prints errors like "tcgetpgrp failed: Not a tty", use `--no-browser` or set `FRUITFUL_NO_BROWSER=1` to just print URLs, then open them manually. On WSL, installing `wslview` enables opening URLs in your Windows browser.
...
- Dependency manager: `uv` for installs and `uv run` commands.
...
- Scripts: `scripts/ingest_catalog.py`, `scripts/build_index.py`, `scripts/refresh_stock.py`.
- Run TUI: `python -m tui`.
- Console: `uv run fruitful-console`.
- CI (optional): GitHub Actions for lint/type/test on 3.11/3.12.

## Repo Layout
- Devlogs: `docs/devlog/` (backgrounders and decisions).
- Data (git-ignored): `data/raw/`, `data/tmp/`, `data/cache/`.
- Indexes (git-ignored): `indexes/`.

## Getting Started (dev)
- Prereqs: Python 3.11+ recommended.
- Create config: copy `config/settings.example.yaml` to `config/settings.yaml` and set paths.
...
- Quick refresh (download → ingest → indexes → stock stub): `uv run python scripts/update_catalog_and_indexes.py`.
- Flags: `--skip-download` to reuse the raw JSON; `--skip-refresh` to omit the stock stub.
- Script notes: `scripts/ingest_catalog.py` validates raw JSON and may write `data/processed/catalog.parquet` when Pandas+PyArrow are available.
- Script notes: `scripts/build_index.py` scaffolds `indexes/` and may create a placeholder SQLite FTS5 DB.
- Script notes: `scripts/refresh_stock.py` is a stub for future stock/price metadata updates.
- Script notes: `scripts/derive_from_device_registry.py` generates `config/synonyms.yaml`, `config/compatibility.yaml`, and `eval/gold.yaml` from a private registry.

## Using a Private Device Registry (optional)
- Set an absolute path in `config/settings.yaml` at `external.device_registry_path`, or pass `--registry` on the CLI.
- Run: `python scripts/derive_from_device_registry.py --registry /abs/path/to/device_registry.md`.
- Outputs: `config/synonyms.yaml` and `config/compatibility.yaml` (review and commit if desired), and `eval/gold.yaml` (git-ignored) for local quality evaluation.

## Status
- Early definition phase.
- See `docs/devlog/` for context, including `docs/devlog/2025-08-29-backgrounder.md` and `docs/devlog/2025-08-29-data-source.md`.
